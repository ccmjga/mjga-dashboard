    ...
    <div class="relative">
      ...
      <div class="flex items-center justify-end gap-2 absolute right-5 bottom-2">
        ...
        <button @click="() => {roleUnbindModal?.show();}"
          ...
          type="button">
          <svg class="me-1 -ms-1 w-5 h-5 text-white " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
            viewBox="0 0 24 24">
...
const { getUserInfo, user } = useUserQuery();
const {
	pagination: {
		pageSize,
		currentPage,
...
onMounted(async () => {
	await getUserInfo(Number($route.params.userId));
	await fetchRolesWith(currentPage.value, pageSize.value, {
		name: roleName.value,
		userId: Number($route.params.userId),
		bindState: bindState.value,
